...
orderRouter.post('/', isAuth, expressAsyncHandler(async(req, res) => {
    if (req.body.orderItems.length === 0) {
        ...
        res.status(201).send({ message: 'Comanda creata.', order: createdOrder });
    }
}));
...
orderRouter.put('/:id/pay', isAuth, expressAsyncHandler(async(req, res) => {
    ...
    if (order) {
        ...
        res.send({ message: 'Comanda actualizata!', order: updatedOrder });
    } else {
        res.status(404).send({ message: 'Comanda nu exista!' });
    }
}));
...
orderRouter.put('/:id/deliver', isAuth, isAdmin, expressAsyncHandler(async(req, res) => {
    ...
    if (order) {
        ...
        const updatedOrder = await order.save();
        res.send({ message: 'Comanda livrata!', order: updatedOrder });
    } else {
        res.status(404).send({ message: 'Comanda nu exista!' });
    }
}));
