...
productRouter.post('/', isAuth, isAdmin, expressAsyncHandler(async(req, res) => {
    ...
    res.send({ message: 'Produs creat!', product: createdProduct });
}));
...
productRouter.put('/:id', isAuth, isAdmin, expressAsyncHandler(async(req, res) => {
    ...
    if (product) {
        ...
        res.send({ message: 'Produs actualizat!', product: updatedProduct });
    } else {
        res.status(404).send({ message: 'Produsul nu exista!' });
    }
}));
...
productRouter.delete('/:id', isAuth, isAdmin, expressAsyncHandler(async(req, res) => {
    ...
    if (product) {
        ...
        res.send({ message: 'Produs sters!', product: deletedProduct });
    } else {
        res.status(404).send({ message: 'Produsul nu exista!' });
    }
}));
...
productRouter.post('/:id/reviews', isAuth, expressAsyncHandler(async(req, res) => {
    ...
    if (product) {
        if (product.reviews.find(x => x.name === req.user.name)) {
            return res.status(400).send({ message: 'Deja ai trimis o recenzie!' });
        }
        ...
        const updatedProduct = await product.save();
        res.status(201).send({ message: 'Recenzie creata!', review: updatedProduct.reviews[updatedProduct.reviews.length - 1] });
    } else {
        res.status(404).send({ message: 'Produsul nu exista!' });
    }
}));
